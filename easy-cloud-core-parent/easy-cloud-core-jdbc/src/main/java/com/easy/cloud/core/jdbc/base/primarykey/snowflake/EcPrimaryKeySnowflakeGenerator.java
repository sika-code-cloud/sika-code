...
public class EcPrimaryKeySnowflakeGenerator implements EcBasePrimaryKeyGenerator {
	...
	static {
		workerId = EcPropUtils.getLong("config/snowflakeid", "snowflakeIdWorker.workerId");
		if (workerId == null) {
			workerId = 0L;
		}
		datacenterId = EcPropUtils.getLong("config/snowflakeid", "snowflakeIdWorker.datacenterId");

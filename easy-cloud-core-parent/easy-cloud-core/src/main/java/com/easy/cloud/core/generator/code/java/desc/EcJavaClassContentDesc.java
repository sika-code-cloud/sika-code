package com.easy.cloud.core.generator.code.java.desc;
...
import com.easy.cloud.core.generator.code.base.constant.EcCodeGenerateConstant.EcColumnLabel;
import com.easy.cloud.core.generator.code.base.pojo.desc.EcFileContentBaseDesc;
...
public class EcJavaClassContentDesc extends EcJavaContentDesc {
	...
	public void addImportFullClassType() {
		...
		if (EcCollectionsUtils.isNotEmpty(getAnnotations())) {
			for (EcJavaAnnotationDesc annotation : getAnnotations()) {
	...
	private EcJavaClassContentDesc buildFieldsByDatabaseSources(EcDatabaseDataSources databaseDataSources) {
		...
		try {
			while (resultSet.next()) {
				String name = EcStringUtils.replaceUnderLineAndUpperCase(resultSet.getString(EcColumnLabel.COLUMN_NAME));
				// id字段略过
				...
				// 设置属性类型
				fieldContentDesc.setSimpleClassType(databaseDataSources.getFieldTypeByColumnType(resultSet.getString(EcColumnLabel.TYPE_NAME)));
				// 设置属性名称
				fieldContentDesc.setName(name);
				// 设置属性完整类型
				fieldContentDesc.setFullClassType(EcCodeGenerateUtils.getCompleteClassName(fieldContentDesc.getSimpleClassType()));

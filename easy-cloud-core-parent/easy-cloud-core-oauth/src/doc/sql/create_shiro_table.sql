...
`available` int(11) NOT NULL DEFAULT '1' COMMENT '可用标志',
`is_deleted` int(11) NOT NULL DEFAULT '0' COMMENT '逻辑删除标志(1:删除)',
`url_pattern` varchar(255) COLLATE utf8_bin DEFAULT NULL COMMENT 'url的匹配模式',
`filter_name` varchar(255) COLLATE utf8_bin DEFAULT NULL COMMENT '过滤器的名称，多个过滤器名称使用英文的,隔开',
`priority_level` int(11) DEFAULT NULL COMMENT '优先级别 数字越大 优先级越高 从1开始',
PRIMARY KEY (`id`),
UNIQUE KEY `unique_url_pattern-filter_name` (`url_pattern`,`filter_name`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='过滤配置信息表';

...
public class EcXMLUtils {
	...
	public static Map<String, Object> getMapFromXmlStr(String content) {
		if (EcStringUtils.isEmpty(content)) {
			return null;
		}
		try {
			InputStream in = new ByteArrayInputStream(content.getBytes(EcCharset.UTF_8));
	...
	public static Map<String, Object> getMapFromInputStream(InputStream inputStream) {
		if (EcBaseUtils.isNull(inputStream)) {
			return null;
		}
		try {
	...
	public static String getXmlStrFromMap(Map<String, Object> parameters) {
		...
		for (String key : parameters.keySet()) {
			if (EcStringUtils.equalsIgnoreCase("attach", key) || EcStringUtils.equalsIgnoreCase("body", key)
					|| EcStringUtils.equalsIgnoreCase("sign", key)) {

package com.easy.cloud.core.jdbc.handler;
...
public class EcJdbcTemplateHandler {
	...
	public void setJdbcTemplate(JdbcTemplate jdbcTemplate) {
		EcJdbcTemplateHandler.jdbcTemplate = jdbcTemplate;
	}
    
    @PersistenceContext
	public void setEntityManager(EntityManager entityManager) {
		EcJdbcTemplateHandler.entityManager = entityManager;
	}
    /**
     * 
     * <p>
...
//    	传入对象的id
    	Long id = null;
    	if(obj instanceof EcBaseEntity){
    		id = ((EcBaseEntity) obj).getId();
    	}
    	
//    	保存数据库中获取到的对象
    	T tFromDataBase = null;
//    	id不为空，执行查询
    	if(EcBaseUtils.isNotNull(id)) {
    		tFromDataBase = (T) entityManager.find(obj.getClass(), id);
    	}
//    	数据库中存在对应id的数据，执行更新 否则执行保存操作
    	if(EcBaseUtils.isNotNull(tFromDataBase)){
    		return entityManager.merge(EcBaseUtils.copeFromObjToTargetObj(obj, tFromDataBase));
    	}else{
    		return entityManager.merge(obj);
    	}
    }
    ...
    public static <T> List<T> findTList(String sql, List<Object> list, Class<T> clazz){
		if(EcCollectionsUtils.isEmpty(list)){
			list = new ArrayList<>();
		}
		List<T> retList = new ArrayList<>();
		
		if(EcStringUtils.isEmpty(sql)){
			return retList; 
		}
    ...
    public static <T> T findTObj(String sql, List<Object> list, Class<T> clazz){
    	...
    	if(EcCollectionsUtils.isNotEmpty(fromDataBaseList)){

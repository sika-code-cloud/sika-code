package com.easy.cloud.core.common.qrcode.utils;
 ...
 */
public class EcQrCodeUtil {

	private static final int BLACK = 0xFF000000;
	private static final int WHITE = 0xFFFFFFFF;

	private EcQrCodeUtil() {
	...
	public static void writeInfoToJpgFile(String content, String fileUrl) {
		...
		hints.put(EncodeHintType.CHARACTER_SET, EcCharset.UTF_8);
		try {
			BitMatrix bitMatrix = multiFormatWriter.encode(content, BarcodeFormat.QR_CODE, 250, 250, hints);
			File file1 = new File(fileUrl);
			EcQrCodeUtil.writeToFile(bitMatrix, "jpg", file1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static BufferedImage writeInfoToJpgBuff(String content) {
		...
		try {
			BitMatrix bitMatrix = multiFormatWriter.encode(content, BarcodeFormat.QR_CODE, 250, 250, hints);
			re = EcQrCodeUtil.toBufferedImage(bitMatrix);
		} catch (Exception e) {
			e.printStackTrace();
		}

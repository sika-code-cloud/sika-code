...
import com.dq.easy.cloud.module.common.encrypt.sm3.pojo.bo.DqSM3DigestBO;
...
public class DqSecureUtil {
	...
	private static byte[] sm3(byte[] data) {
		...
		DqSM3DigestBO sm3 = DqSM3DigestBO.newInstance();
	...
	public static String sm3X16Str(String data, String encoding) {
		byte[] bytes = new byte[DqSM3DigestBO.newInstance().getDigestSize()];
		try {
			bytes = DqSecureUtil.sm3(data.getBytes(encoding));
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}

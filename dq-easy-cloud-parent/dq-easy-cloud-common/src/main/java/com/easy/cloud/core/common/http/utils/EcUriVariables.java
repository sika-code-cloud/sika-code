package com.dq.easy.cloud.module.common.http.utils;
...
import com.dq.easy.cloud.module.common.http.constant.DqHttpErrorCodeEnum;
...
public class DqUriVariables {
	...
	private static void putKeyValueToMap(StringBuilder temp, boolean isKey, String key, Map<String, Object> map) {
		if (isKey) {
			key = temp.toString();
			if (key.length() == 0) {
				throw DqBaseBusinessException.newInstance(DqHttpErrorCodeEnum.HTTP_CONTENT_FORMAT_WRONG);
			}
			map.put(key, "");
		} else {
			if (key.length() == 0) {

...
public class DqWxPayLogic extends DqBaseLogic {
	...
	public DqBaseServiceResult wxJsapiPay(DqPayOrderDTO dqPayOrderDTO) {
		DqPayOrderBO dqPayOrderBO = new DqWxPayOrderBO(dqPayOrderDTO, DqWxTransactionType.JSAPI).initDqPayOrderDTO();
		// 微信公众号支付数据校验
...
//		获取支付订单逻辑对象
		DqPayOrderBO dqPayOrderBO = new DqWxPayOrderBO(dqPayOrderDTO, DqWxTransactionType.NATIVE).initDqPayOrderDTO();
//		数据校验
		dqPayOrderBO.verifyGeneratePayQrCodeData();
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ImageIO.write(dqWxPayService.generatePayQrCode(dqPayOrderDTO), DqImageFormat.JPEG, baos);
